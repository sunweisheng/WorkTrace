...
- `config/collected_merge.json` 控制高风险复核；多条候选在来源事件达到 10 条、来源文件达到 4 个、跨批、分组修复或不同非空工作流时触发复核，单条候选直接保留。跨批只协调存在共同消息指纹或共同文件的候选；最多三路复核并保持原候选顺序。拆开已确认多条候选时必须返回非空原因，否则保留原组并记录告警。
