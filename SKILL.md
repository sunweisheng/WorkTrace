...
- 多人候选阶段默认使用完整事件正文并受 `max_model_input_tokens=6200` 统一约束；限制按最终文字提示词、在线请求追加的 `/no_think`、完整 JSON Schema 和结构化输出包装的合计输入估算执行。超过上限时关系优先分批，局部分组在原有模型调用中同时返回跨批候选摘要，不为每个组单独增加调用；不可继续拆分时直接失败，不调用模型且不重试。
...
- 个人日报和多人汇总的模型输入统一受默认值为 `6200` 的 `max_model_input_tokens` 约束，不另设多人合并字符阈值；分批和调用前检查都必须计算客户端已知的最终提示词、在线请求追加的 `/no_think`、完整 JSON Schema 和结构化输出包装。会话分段窗口、锚点降级批次、跨会话候选合并及多人汇总等仍可拆的组合输入必须继续拆分，不能因为超限直接结束；单条事件片段、单个临时协作复核候选、单个事实复核候选或最小必要输入仍超限时才失败，且不能按普通错误重试。该值是调用前估算上限，不冒充服务端响应中的精确 `usage.input_tokens`。
