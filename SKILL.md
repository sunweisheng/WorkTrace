...
9. 在线请求成功返回、但结果未通过 Python 证据或结构校验时，先按程序现有配置用 Online 局部重试当前请求；结果质量错误在局部重试用尽后，只把当前请求交给 Codex 再执行一次。Codex 结果合法时继续流程，下一请求仍优先 Online；Codex 失败或结果仍不合法时立即停止整次生成且不写文件，取消并发队列中尚未开始的请求，并让已经在途的请求完成当前调用后不再进入新的局部重试或 Codex。不得擅自增加重试次数、重新运行整次流程或把整次流程切换到 Codex。
