...
- 多人候选阶段默认使用完整事件正文，并按 `model_input_batch_target_tokens=5200` 的完整输入估算优先分批；估算包含最终文字提示词、在线请求追加的 `/no_think`、完整 JSON Schema 和结构化输出包装。超过目标时关系优先分批，局部分组在原有模型调用中同时返回跨批候选摘要，不为每个组单独增加调用；已经拆到最小必要输入仍超过目标时标记为 `oversized_singleton` 并发送，不设置额外的本地绝对上限。
- `config/collected_merge.json` 控制高风险复核和合法合并原因；`shared_file` 只表示同一个文件，不包含同类但不同的文件，属于同一报送周期、提交任务或成套交付材料的不同文件使用 `same_deliverable_batch`。多条候选在来源事件达到 10 条、来源文件达到 4 个、跨批、分组修复、不同非空工作流，或同一会话连接了多个没有共同消息或共同文件的部分时触发复核，单条候选直接保留。跨批只协调存在共同消息指纹或共同文件的候选；后一类复核中，多事件子组必须有共同消息、共同文件或配置允许的语义依据，不能仅因同一会话保留。最多三路复核并保持原候选顺序。拆组时只需一条顶层 `split_reason` 说明整体业务差异；旧记录中任一子组已有理由也兼容接受，完全没有理由时才保留原组并记录告警。
...
- 个人日报和多人汇总统一使用默认值为 `5200` 的 `model_input_batch_target_tokens` 作为分批目标，不另设多人合并字符阈值；分批和调用前检查都必须计算客户端已知的最终提示词、在线请求追加的 `/no_think`、完整 JSON Schema 和结构化输出包装。会话分段窗口、锚点降级批次、跨会话候选合并及多人汇总等仍可拆的组合输入必须继续拆分；单条事件片段、单个临时协作复核候选、单个事实复核候选或其他最小必要输入仍超过目标时允许发送，并在调试记录中保存估算 token、目标值和超出量。模型服务明确拒绝输入时整次生成失败，不重试该请求且不写半成品。该值是本地分批目标，不是服务端上下文上限，也不冒充响应中的精确 `usage.input_tokens`。
