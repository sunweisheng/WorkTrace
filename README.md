...
当前阶段，WorkTrace 主要服务使用 `Codex` 或兼容 `Codex Skill` 的 Agent 的员工用户。个人日报模式的输入来源默认是飞书聊天，输出是一份当天的本地 Markdown 文件，并通过飞书 CLI 机器人身份发给员工自己。另有管理人员汇总模式，可把多人已经生成的 WorkTrace Markdown 放入 `merge_inbox/YYYY/MM/DD/` 后生成团队 `_merged.md`；日期目录下的一级子目录会作为独立合并范围，各自生成子目录内的 `_merged.md`。
...
7. 其它事件交给 LLM 保守判断是否属于同一真实工作事件，并输出合并后的保留理由和依据
8. Python 校验和修复 LLM 返回的分组，保留来源人员和来源事件 ID，并在写入前再次校验保留门槛
9. 每个合并范围生成本目录 `_merged.md`
10. 若配置了 `config/merge_delivery.local.json`，则根目录结果上传到飞书 Drive 的 `YYYY/MM/DD/`，子目录结果上传到 `YYYY/MM/DD/<子目录名>/`
...
- 管理汇总 CLI 固定为 `python -m src.worktrace.cli merge-collected --date YYYY-MM-DD`，输入来自 `merge_inbox/YYYY/MM/DD/` 及其一级子目录，每个合并范围生成本目录 `_merged.md`。
...
汇总前会先过滤未通过结构化保留门槛的来源事件；汇总 LLM 输出的每个 group 也必须带 `retention_reason` 和 `retention_detail`。其中 `retention_detail` 作为保留依据/来源证据使用，最终 `_merged.md` 写入前会再次校验。
...
日期根目录会生成一个 `_merged.md`；日期目录下每个一级子目录也会单独生成自己的 `_merged.md`。每个合并范围只读取当前目录下的普通 `.md` 文件，跳过 `_merged.md`、隐藏文件、非 Markdown 文件和更深层子目录。
...
merge_inbox/2026/06/29/项目A/_merged.md
```

如需上传到飞书 Drive，可复制 [config/merge_delivery.example.json](/Users/sunweisheng/Documents/GitHub/WorkTrace/config/merge_delivery.example.json) 为本地 `config/merge_delivery.local.json`，并填写目标文件夹 URL。根目录结果上传到 `YYYY/MM/DD/`，子目录结果上传到 `YYYY/MM/DD/<子目录名>/`。实际配置文件不会提交到 Git。
...
成功时，`stdout` 会返回包含 `source_file_count`、`source_event_count`、`merged_event_count`、`warning_messages`、`upload_status` 和 `outputs` 的 JSON 摘要；`outputs` 会列出每个合并范围的输入目录、输出文件和上传结果。
