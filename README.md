...
候选发现默认发送来源 MD 中的完整事件正文，不再按原始聊天消息的 `prompt_message_char_limit` 固定截取。估算输入超过 `max_model_input_tokens` 时，系统按消息、文件和会话关系优先分批；只有不可继续拆开的输入才按来源平均保留开头和结尾内容。局部分组模型在原有调用中同时返回多成员组的候选摘要，Python保留全部来源和指纹，再用摘要做跨批汇合判断，因此正常路径不增加“每组摘要”调用。正式内容仍从原始事件完整正文生成，并按已锁定候选组分批处理。
