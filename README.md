...
候选、复核和正式正文请求统一受 `max_model_input_tokens=6200` 限制。超限时按消息、文件和会话关系优先分批，必要时复用正文切片和分层摘要。正式正文必须返回完整 `covered_draft_ids` 和带来源的 `fact_items`；Python 只重试当前组，重试后仍不完整则当前 scope 失败，不写不完整文件。单条事件组直接保留原文，不增加模型调用；正文不再把全部来源原文机械追加到模型结果。
