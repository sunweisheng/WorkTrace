...
事实证据不完整、来源消息达到 8 条、来源参与人达到 3 人，或命中配置中的多个对象、对比案例、多个地点、责任归属、推断决策等风险信号时，系统在跨会话合并前增加一次个人事实局部复核。复核模型重新读取原聊天，可以确认原文，也可以删除或改写无证据内容；复杂、多步骤事件本身不是删除理由。Python 校验复核结果完整覆盖当前批次、每个事实只引用合法消息、正文与事实项完全一致，再按固定规则接收修订；确认没有任何受支持事实时按配置删除。协议错误只重试当前批次，重试失败仍然不写文件。
