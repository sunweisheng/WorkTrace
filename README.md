...
当前阶段，WorkTrace 主要服务使用 `Codex` 或兼容 `Codex Skill` 的 Agent 的员工用户。个人日报模式的输入来源默认是飞书聊天，输出是一份当天的本地 Markdown 文件，并通过飞书 CLI 机器人身份发给员工自己。另有管理人员汇总模式，可把多人已经生成的 WorkTrace Markdown 放入 `merge_inbox/YYYY/MM/DD/` 后生成团队 `_merged.md`。
...
- 管理汇总模式可选上传 `_merged.md` 到指定飞书 Drive 文件夹
...
12. 将事件列表写入当天 Markdown 文件
13. 通过飞书 CLI 机器人身份将当天 Markdown 文件发送给当前登录用户自己

管理人员汇总流程如下：

1. 管理人员把多人已经生成的 WorkTrace Markdown 放入 `merge_inbox/YYYY/MM/DD/`
2. 文件名使用 `YYYY-MM-DD-姓名.md`
3. Python 读取标准 WorkTrace Markdown 事件块
4. Python 先按同一套结构化保留门槛过滤来源事件
5. 相同 `event_id` 且标题/内容满足确定性规则时先锁定为合并组
6. 其它事件交给 LLM 保守判断是否属于同一真实工作事件，并输出合并后的保留理由和依据
7. Python 校验和修复 LLM 返回的分组，保留来源人员和来源事件 ID，并在写入前再次校验保留门槛
8. 生成同目录 `_merged.md`
9. 若配置了 `config/merge_delivery.local.json`，则在飞书 Drive 目标文件夹下创建 `YYYY/MM/DD/` 并上传 `_merged.md`
...
管理人员汇总产物 `_merged.md` 例外：为了团队事项审阅和追溯来源，会额外显示 `来源人员` 和 `来源事件 ID`。

“有效工作事件”必须能说明为什么值得沉淀：例如产出或修改了文档、数据、配置、代码，形成了结论或决策，发现或处理了问题/风险，明确了待办、负责人、期限，推进了客户、合同、付款、交付等外部业务，或审核/审批有明确对象和结论。仅写“完成审核”“完成审核工作”，或只是“下午开会互通一下信息”的普通安排，不应进入个人日报，也不会进入管理人员汇总。
...
- 管理汇总 CLI 固定为 `python -m src.worktrace.cli merge-collected --date YYYY-MM-DD`，输入来自 `merge_inbox/YYYY/MM/DD/`。
...
事件是否值得保留由结构化字段和 Python 门槛判断，不靠维护大型关键词表。如果需要精确排除某些已确认无价值的事件标题或内容特征，请单独维护规则文件 [config/event_rules.json](/Users/sunweisheng/Documents/GitHub/WorkTrace/config/event_rules.json)：
...
这两项只适合少量精确拉黑，不承担“判断事件价值”的职责。这里建议只放“精确匹配、且确认应排除”的词，避免误伤正常工作事件。
...
汇总前会先过滤未通过结构化保留门槛的来源事件；汇总 LLM 输出的每个 group 也必须带 `retention_reason` 和 `retention_detail`，最终 `_merged.md` 写入前会再次校验。
...
```text
merge_inbox/2026/06/29/_merged.md
```

如需上传到飞书 Drive，可复制 [config/merge_delivery.example.json](/Users/sunweisheng/Documents/GitHub/WorkTrace/config/merge_delivery.example.json) 为本地 `config/merge_delivery.local.json`，并填写目标文件夹 URL。实际配置文件不会提交到 Git。
...
成功时，`stdout` 会返回包含 `source_file_count`、`source_event_count`、`merged_event_count`、`warning_messages` 和 `upload_status` 的 JSON 摘要。
