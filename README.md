...
事实证据不完整、来源消息达到 8 条、来源参与人达到 3 人，或命中配置中的多个对象、对比案例、多个地点、责任归属、推断决策等风险信号时，系统在跨会话合并前增加一次个人事实局部复核。复核输出使用固定的标题、正文、主要动作、具体对象、保留依据和工作流证据位置，不能通过缩短可变列表省略字段。复核模型重新读取原聊天，可以确认原文，也可以删除或改写无证据内容；复杂、多步骤事件本身不是删除理由。Python 校验复核结果完整覆盖当前批次、每个事实只引用合法消息、正文与事实项完全一致，再按固定规则接收修订；确认没有任何受支持事实时按配置删除。协议错误只重试当前批次，重试失败仍然不写文件。
...
`scripts/replay_day_with_trace.py` 的 `summary.json` 会把两类复核文件的路径、统计、尝试次数和失败次数写入 `review_artifact_summary`。`scripts/report_replay_call_inputs.py` 会把每次临时协作复核和个人事实复核（包括失败重试）作为独立模型调用列入 `call-input-report.md`。耗时报告按通用 `request_kind` 读取，自动识别 `retention_review` 和 `personal_fact_review`。
