...
命中任一条件时增加一次模型复核。确认同一事项时保留原组；发现不同事项时拆组；拿不准时拆开。若复核原因是同一会话连接了多个无共同消息或文件的部分，每个保留的多事件子组还必须有共同消息、共同文件、同一对象或连续动作依据，不能只写同一会话；否则重试当前复核。复核必须完整覆盖原组全部 draft ID，遗漏、未知或重复 ID 只重试当前复核，仍无效则当前 scope 失败。
