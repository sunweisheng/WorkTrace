...
第一阶段只发送事件 ID、标题、具体对象、工作流、动作和受限长度内容，由模型返回候选分组；同一会话、共同消息和共同文件只提供候选关系，不锁定模型结果。第二阶段按已确认组发送完整内容并生成正式汇总，单条组直接保留。

- 候选发现不超过 `collected_merge_prompt_char_threshold` 时一次处理
- 超过阈值时按消息、文件和会话关系集合优先分批，再用组摘要做跨批汇合
- 正式内容生成按锁定候选组装箱，不拆开能放入阈值的关系组
- 单个正式组超过阈值时分层汇总，直到得到一个保留全部来源的正式事件
