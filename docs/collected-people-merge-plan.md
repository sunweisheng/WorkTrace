...
- 每个滚动批次或高风险复核请求在内存中把数量大于零的共同消息、共同文件关系编号为 `MSG-xxx`、`FILE-xxx`；编号清单进入上下文和 `evidence_relation_ids` 枚举，不创建临时目录或独立文件
- Python 按同日会话指纹形成 `conversation_groups`，只发送临时组编号和 draft ID，不发送会话哈希或原始会话 ID；它只是候选关系，不编号、不进入证据枚举
- 证据编号、相同具体对象或连续动作是强证据，但只能证明真实共享关系存在；即使指纹集合完全相同也不能自动合并，仍由模型结合具体对象和前后动作确认
...
第一阶段发送内部草稿 ID、来源人员、来源负责人、标题、具体对象、工作流、动作、完整事件正文和当前证据编号清单。模型只返回 `semantic_reasons`、`evidence_relation_ids`、`reason_detail` 和 `risk_flags`，不能直接返回 `shared_message`、`shared_file` 或内部 `group_reason`。Python 检查编号存在、编号关联事件属于当前组，以及只依靠证据编号合并时所选关系能连接组内全部成员；通过后才恢复内部 `shared_message`、`shared_file`，并与合法语义理由组合为现有 `group_reason`。无真实关系时 `evidence_relation_ids.maxItems=0`。多成员组在同一次调用中同时返回候选标题、候选内容和候选对象，单成员组由 Python 保留原事件。
...
命中任一条件时增加一次模型复核。确认同一事项时保留原组；发现不同事项时拆组；拿不准时拆开。每个保留的多事件子组必须有合法语义理由或经过 Python 反查的证据编号，并分别提供自己的 `reason_detail`；单条组不要求 `reason_detail`。拆组时返回一条顶层 `split_reason` 解释全部子组的业务差异，不要求各子组重复填写；拆成多个单条组，或一个多事件组加若干单条组时，都只需这一条整体理由。旧记录中任一子组的非空 `split_reason` 仍兼容接受；如果确实拆分但所有位置都无理由，拒绝本次拆分、保留原候选并记录告警。复核必须完整覆盖原组全部 draft ID，遗漏、未知、重复 ID、非法证据、证据越界或关系未覆盖只重试当前复核，并把具体错误码、字段位置、组 ID 和编号反馈给模型。
...
429、HTTP 5xx、连接、超时、流式 Function 参数异常、空返回和无效 Function 参数时，当前文字请求立即切到 Codex，不重复在线调用，下一请求仍优先在线。Codex 失败则当前阶段失败，不循环调用；鉴权、权限、TLS 和参数错误不会切换。字段缺失、来源覆盖、事实覆盖和证据关系错误属于结果质量校验，仍只对当前批次或当前组局部重试。
...
- 来源文件/事件指标
- 本轮阶段类型、Function 名、去敏定义、证据编号清单、完整 `input_events` 和 `deterministic_groups`；`conversation_groups` 只作为候选上下文
- 原始 group 和字段缺失统计
- 高风险复核触发原因、复核前后分组、有效拆分理由及来源、Python 校验结果和来源覆盖
...
针对高风险复核失败素材，可运行 `python3 scripts/replay_collected_review_failures.py` 离线回放 M07-M11；加 `--output-dir` 生成当前 prompt、Function 定义、证据编号和汇总，加 `--result-dir` 校验后续实验返回。脚本不调用在线模型。
