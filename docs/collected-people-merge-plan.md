...
候选、复核和正式内容请求统一按最终提示词、在线请求追加的 `/no_think`、完整 JSON Schema 和结构化输出包装的合计输入估算受 `max_model_input_tokens=6200` 限制。分批和实际调用前使用同一估算口径；不可继续拆分且仍超限时直接失败，不调用模型且不重试。该值覆盖调用前客户端已知输入，服务端精确 token 仍以响应 usage 为准。
...
- 候选发现完整输入估算不超过 `max_model_input_tokens` 时一次处理
...
- 文字提示词估算 `prompt_estimated_tokens`、包含 JSON Schema 的完整输入估算 `input_estimated_tokens`、`max_model_input_tokens` 上限和辅助字符数
