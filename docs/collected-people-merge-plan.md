...
候选、复核和正式内容请求统一按最终提示词、在线请求追加的 `/no_think`、完整 JSON Schema 和结构化输出包装估算，以 `model_input_batch_target_tokens=5200` 为分批目标。分批和实际调用前使用同一估算口径；不可继续拆分的最小输入仍超过目标时标记后发送，由模型服务决定是否接受。该值覆盖调用前客户端已知输入，服务端精确 token 仍以响应 usage 为准。

第一阶段发送内部草稿 ID、来源人员、来源负责人、标题、具体对象、工作流、动作和完整事件正文，由模型返回候选分组、`group_reason` 和 `risk_flags`；合法合并原因及定义从 `config/collected_merge.json` 读取。`shared_file` 只表示同一个文件，不包括同格式或同类的不同文件；同一报送周期、提交任务或成套交付材料使用 `same_deliverable_batch`。多成员组在同一次调用中同时返回候选标题、候选内容和候选对象，单成员组由 Python 保留原事件。同一会话、共同消息和共同文件只提供候选关系，不锁定模型结果。
...
命中任一条件时增加一次模型复核。确认同一事项时保留原组；发现不同事项时拆组；拿不准时拆开。若复核原因是同一会话连接了多个无共同消息或文件的部分，每个保留的多事件子组还必须有共同消息、共同文件，或配置允许的语义依据，不能只写同一会话；否则重试当前复核。拆组时返回一条顶层 `split_reason` 解释全部子组的业务差异，不要求各子组重复填写；兼容读取旧记录中任一子组的非空理由。复核必须完整覆盖原组全部 draft ID，遗漏、未知或重复 ID 只重试当前复核，仍无效则当前 scope 失败。
...
429、HTTP 5xx、连接、超时、流式 JSON、空返回和无效 JSON 时，当前文字请求立即切到 Codex，不重复在线调用，下一请求仍优先在线。Codex 失败则当前阶段失败，不循环调用；鉴权、权限、TLS 和参数错误不会切换。字段缺失、来源覆盖和事实覆盖属于结果质量校验，仍只对当前批次或当前组局部重试。
...
- 文字提示词估算 `prompt_estimated_tokens`、包含 JSON Schema 的完整输入估算 `input_estimated_tokens`、分批目标 `input_target_tokens`、`oversized_singleton` 和辅助字符数
...
针对高风险复核失败素材，可运行 `python3 scripts/replay_collected_review_failures.py` 离线回放 M07-M11；加 `--output-dir` 生成当前 prompt、Schema 和汇总，加 `--result-dir` 校验后续实验返回。脚本不调用在线模型。
