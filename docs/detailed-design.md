...
- 分析范围限定为“当天自己发过消息的会话”。
- 消息读取策略为先抓取整会话当日消息，再围绕本人发言锚点在本地按消息条数和回复关系裁剪上下文。
...
表示围绕本人发言锚点裁剪后的上下文切片，用于作为 LLM 的基本分析单元。切片边界优先由消息条数和回复关系确定，而不是由时间窗口确定。一个切片可包含多个连续消息，并保留来源会话信息。
...
- `date` 仅保留 `YYYY-MM-DD`。
- `result` 可为空字符串。
- `event_id` 由脚本基于日期和排序后的源消息 ID 集合生成，只要不重复即可。
- 不长期保存原始消息正文。
...
首版上下文切片规则不采用时间窗口，而采用“消息条数优先，回复关系补全”的方式。

### 10.1 锚点与锚点簇

- 本人在目标日期内发出的每条消息，先视为一个原始锚点。
- 连续或紧邻的本人消息应先合并为一个锚点簇，再围绕锚点簇裁剪切片。
...
这里的“前后各最多 50 条、基础总上限 150 条”用于保证本人回复较晚时仍能保留足够前文，同时控制单个切片在首轮分析时不会无限增长。
...
也就是说，切片不仅包含锚点簇前后固定条数的消息，还要补齐与锚点簇有明确引用关系、回复关系的消息。首版引用链和回复链只追 1 层直接关系，不做递归深追。
...
- 一旦进入这种“按 LLM 请求补充上下文”的二次扩展流程，基础 `150` 条上限不再生效，以满足该事项的补充分析需要为准。

### 10.7 切片阶段不做的判断

切片阶段只负责边界完整性，不负责语义归纳：

- 不在切片阶段判断是否属于工作事项。
- 不在切片阶段做跨会话合并。
- 不让 LLM 决定切片边界，边界由 Python 脚本按规则确定。

## 11. 附件处理策略

- 默认只使用可直接读取的消息文本、卡片文本、链接标题等内容。
- 是否需要补读附件文本由 LLM 判断；当 LLM 明确请求附件内容时，Python 再补读附件文本并追加到后续分析输入中。
- 首版不做图片 OCR。
