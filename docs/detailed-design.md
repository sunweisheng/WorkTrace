...
管理人员汇总模式的目标是：在不重新读取员工原始聊天的前提下，合并多人已经生成的 WorkTrace Markdown，形成团队视角 `_merged.md`。
...
- 管理人员对 `merge_inbox/YYYY/MM/DD/` 下多人 WorkTrace Markdown 做同日合并
...
- 管理人员汇总模式会把已收集 Markdown 的事件标题、内容、来源人员和来源事件 ID 发送给 LLM 做合并判断，不读取原始聊天
...
- `stdout` 返回 `CollectedMergeRunResult` 的 machine-readable JSON
