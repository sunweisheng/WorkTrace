...
全日分组 Function 每组只返回 `draft_ids`、`primary_draft_id`、`merge_reason` 和 `evidence_message_ids`，稳定 `group_id` 由 Python 按候选顺序生成。Python 校验每个候选恰好出现一次、主事件属于当前组、证据属于组内候选的来源消息；多事件组必须有具体合并理由和合法证据。全部单例是合法结果。
