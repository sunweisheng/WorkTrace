...
- WorkTrace 会把经过裁剪和压缩后的必要文本上下文发送到你配置的在线模型服务
- 如果图片摘要已启用，符合数量和大小限制的聊天图片也会临时下载并发送到同一在线模型服务生成摘要
...
管理人员开启多人汇总 trace 后，每个 step JSON 还会保存本轮完整来源事件、确定性分组和工作流强制拆组 warning，便于判断“哪些共同证据支持合并”或“为什么被拆开”。
