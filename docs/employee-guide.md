...
候选事件形成后，模型按同一实际事项做全日分组。分组提示词不发送会话 ID 和片段 ID；同一会话不同事项、同类工作和宽泛目标都不能单独支持合并。每个多事件组必须写明一个具体共同对象，并逐条说明每个候选与该对象的直接关系，引用该候选自己的来源消息。Python 检查候选完整、成员说明一一对应和证据归属。共享来源片段、直接 reply/quote、共同消息或共同文件跨越已有分组时，系统会增加局部复核；同一会话本身不会触发。局部复核只能合并完整现有组，不能拆散已通过检查的组。分组说明和反例来自 `config/event_grouping.json`，Python 不读取聊天文字判断业务含义。
