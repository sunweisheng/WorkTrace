...
“主要动作”表示方案确认、配置修改、执行验证等实际动作；“本人参与方式”表示你在事件中是发起、主责执行、协作参与、确认决策、反馈验收、被指派或参与回应。主要动作或本人参与方式没有足够证据时显示“未明确”。
...
每条个人事件还会为标题、正文、主要动作、具体对象和保留依据保存内部事实证据。消息较多、参与人较多、事实证据不完整，或模型识别到多个对象、对比案例、多个地点、责任归属等风险时，系统会让模型重新对照原聊天确认当前事件。它可以删除或改写没有依据的地点、角色、结论和建议，但不会因为事情复杂、参与人多或步骤多就删除真实工作。Python 不理解这些文字的业务含义，只核对模型是否完整返回、每项事实是否引用当前聊天的真实消息，以及修订后的正文是否被事实项完整覆盖。

候选事件形成后，模型按同一实际事项做全日分组，Python 检查每个候选恰好出现一次、主事件和证据都属于当前组。共享来源片段、直接 reply/quote、共同消息或共同文件跨越已有分组时，系统会增加局部复核；同一会话本身不会触发。局部复核只能合并完整现有组，不能拆散已通过检查的组。分组说明来自 `config/event_grouping.json`，Python 不读取聊天文字判断业务含义。
...
- `day_group_review.json` 中的强关联组件、局部复核尝试和保留决定
...
使用 `scripts/replay_day_with_trace.py` 回放时，`summary.json` 的 `review_artifact_summary` 会汇总两类事实复核文件，`day_grouping_artifact_summary` 和 `day_grouping_summary` 会汇总全日分组，`llm_usage_summary` 会按调用类型汇总次数、token 和耗时。调用输入报告会逐次列出复核及其失败重试，并把图片摘要与文字调用分开统计。事实复核并发阶段分析实际运行耗时时看 `personal_fact_review_all` 墙钟值；强关联复核看 `day_group_review_all`，整个分组阶段看 `merge_day_candidates`，各候选或请求耗时之和只代表模型调用总负载。复核文件不额外复制整段聊天，完整上下文仍从已有分段调试输入查看。

需要比较新旧分组时，先保存旧 trace，再运行 `scripts/report_event_grouping_comparison.py`。脚本只统计候选覆盖、单例/多事件组、发生合并或拆分的候选、复核结果、理由和证据，不代替人工判断事件是否真的应当合并。
