...
全日分组继续使用已经通过临时协作复核和事实复核的 `SourceBackedEventDraft`。每个候选至少提供：
...
```json
{
  ...
  "merge_reason": "两条候选分别记录同一交付物的方案确认和执行反馈。",
  "evidence_message_ids": ["om_xxx", "om_yyy"]
}
```

模型不返回 `group_id`。Python 按候选原始顺序生成 `group-001`、`group-002` 等稳定内部编号。
...
- `evidence_message_ids` 只能引用当前组候选的来源消息。
- 多事件组必须有非空 `merge_reason` 和至少一条合法来源消息证据。
- 单例组允许使用固定单例理由和空合并证据。
