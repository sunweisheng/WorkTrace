...
1. Codex 首次返回非法结果时，把具体缺失、重复、主事件或证据错误反馈给 Codex，并按配置只重试当前请求。
2. Codex 的结果质量重试用尽后，把同一输入和最新错误交给 Online 一次。
3. Online 返回合法结果时继续。
4. Online 成功返回但结果仍非法时，保留其中互不冲突且完全合法的组；所有受错误影响的候选拆成单例，并记录 warning。
5. Online 技术调用失败时终止整次生成，不写 Markdown。

Codex 和 Online 的技术线路重试仍遵守通用 failover 约定；下一项模型请求重新优先 Codex。
...
完整输入超过当前预算 profile 的目标时，系统按候选分批完成初步分组，再由 Python 直接合并所有批次结果、校验全量覆盖并生成稳定编号。配置不存在或当前主备模型组合没有匹配项时，目标回退为 `7000`。这里不构造候选摘要，也不做摘要再次分组；跨批可能漏掉的关系由下面面向全部组的标题发现和完整内容复核处理。
...
标题发现使用统一 Online/Codex 输入估算。完整编号和标题清单超过当前预算 profile 的目标时仍作为不可拆输入整体提交并标记超限。技术失败或结果非法时只处理当前请求：Codex 按现有机制重试，仍失败或非法时交给 Online 一次；全部失败后按没有标题候选继续，记录 warning，不阻止个人 Markdown 写入和本人送达。
...
最终分组确定后，单成员组和多成员组都由 `personal_group_render` 逐项生成带证据的标题、正文和具体对象，最多三路并行。生成结果必须覆盖组内每个候选；完整过程组使用能够概括全部成员的标题，只有紧密子集成立时才使用更具体的标题。保留理由、涉及文件、本人参与方式、消息覆盖和统计仍由 Python 处理。某项失败时只对该项沿用确定性内容并记录 warning。

### 5.5 失败处理

局部复核结果非法时，按配置对当前范围反馈具体错误并由 Codex 重试；Codex 结果质量重试后仍非法，再把带有最后校验错误的当前范围交给 Online 一次。技术请求已经走完 Codex 重试和 Online 备用仍失败时，不再重新开始一轮 Codex，也不占用结果质量重试。仍非法或技术调用失败时：
...
- 全部最终事件内容重写覆盖锁定成员，失败时只对当前事件确定性回退。
- 标题发现严格只发送编号和标题，超过当前预算 profile 的目标仍整体提交，全部失败后个人日报继续。
- Codex 结果质量重试、Online 备用、非法结果拆单修补和技术失败终止。
- 局部复核并发、失败保留原分组和统计。
