...
> 历史说明：本文涉及的工作流校正已被 [取消工作流概念并改进事件分组](workstream-free-event-grouping-design.md) 替代；其余锚点能力状态按历史保留。
...
实验输出 JSON/表格和调试文件，不写正式个人 Markdown，也不执行正式全日初步分组、全部组标题发现、完整内容复核、多成员内容重写、文件证据聚合和自发送链路。
