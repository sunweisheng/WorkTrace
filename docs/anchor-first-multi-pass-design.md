...
| 旧工作流归属校正 | 曾进入主链，现已取消并由全日事件分组和强关联漏合并复核替代 | `pipeline/day_event_grouping.py` |
...
flowchart LR
    ...
    G --> H["全日 LLM 分组"] --> I["Python 完整性校验"] --> J["强关联漏合并复核"] --> K["最终事件"]
```
...
- 不再使用工作流字段；全日分组直接返回多事件组与单例，Python 校验完整性后只对跨组强关联做局部复核
